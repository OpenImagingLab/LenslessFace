...
class DeepposeRegressionHead(nn.Module):
    ...
    def get_loss(self, output, target, target_weight):
        ...
        # print(target.shape, target_weight.shape)
...
@HEADS.register_module()
class DeepposeRegressionHead_hybridLoss(nn.Module):
    """Deeppose regression head with fully connected layers.
    ...
    def forward(self, x,kernel):
        ...
        N, C = output.shape
        if self.out_sigma:
            return output.reshape([N, C // 4, 4]),kernel
        else:
            return output.reshape([N, C // 2, 2]),kernel

    def get_loss(self, output,kernel, target, target_weight):
        """Calculate top-down keypoint loss.
        ...
        losses['reg_loss'] = self.loss(output,kernel, target, target_weight)
        return losses
    ...
    def inference_model(self, x,kernel, flip_pairs=None):
        """Inference function.
        ...
        Args:
            ...
            flip_pairs (None | list[tuple()):
                Pairs of keypoints which are mirrored.
        """
        output,kernel = self.forward(x,kernel)

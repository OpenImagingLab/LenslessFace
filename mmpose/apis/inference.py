...
def init_hybrid_pose_model(config, checkpoint=None, device='cuda:0'):
    """Initialize a pose model from config file.
    ...
    for bbox in bboxes:
        ...
        data = {
            ...
            'ann_info': {
                ...
                'joint_weights':cfg.dataset_info['joint_weights'],
...
@deprecated_api_warning(name_dict=dict(img_or_path='imgs_or_paths'))
def inference_top_down_hybrid_pose_model(model,
                                  config,
                                  imgs_or_paths,
                                  person_results=None,
                                  bbox_thr=None,
    ...
    if (dataset_info is None and cfg
            and 'dataset_info' in cfg):
    ...
    if person_results is None:
        ...
        if isinstance(sample, str):
            ...
            height, width = sample.shape[:2]
        person_results = [{'bbox': np.array([0, 0, width, height, 1.])}]
...
def vis_pose_result(model,
    ...
    if cfg1 is None:
        ...
        if (dataset_info is None and 'dataset_info' in cfg1):
            dataset_info = DatasetInfo(cfg1.dataset_info)
    if dataset_info is not None:

    ...
    if legend is None:
        ...
        for json_log in args.json_logs:
            for metric in args.keys:
                ...
                basename = json_log.split('/')[-2].split('.')[-1]
...
def plot_phase_train(metric, log_dict, epochs, curve_label, json_log):
    ...
    for epoch in epochs:
        ...
        for i, iter in enumerate(iters):
            if log_dict[epoch]['mode'][i] == 'train':
                train_iters.append(iter)
        num_iters_per_epoch = train_iters[-1]
        assert len(iters) > 0, (
            'The training log is empty, please try to reduce the '
            'interval of log in config file.')
        xs.append(np.array(train_iters) / num_iters_per_epoch + (epoch - 1))
...
def plot_curve(log_dicts, args):
    ...
    if args.out is None:
        # plt.show()
        args.out = args.json_logs[0].split('json')[0] + metrics[0] + '.pdf'
    # else:

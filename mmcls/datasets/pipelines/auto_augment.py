...
@PIPELINES.register_module()
class RotateTrans(Rotate):
    """Rotate images.
    with store the affine matrix
    ...
    def __call__(self, results):
        ...
        results['affine_matrix'] = np.array(
            [[cosx, sinx, 0],
             [-sinx, cosx, 0]]).astype(np.float32)
...
class AffineRTS(object):
    ...
    def __call__(self, results):
        ...
        matrix1 = np.concatenate((matrix,np.array([[0.0, 0.0, 1.0]])), axis=0)
        matrix2 = np.linalg.inv(matrix1)
        matrix3 = matrix2[0:2,:] 
        results['affine_matrix'] = matrix3.astype(np.float32)
...
class TorchAffineRTS(object):
    ...
    def __init__(self,
                 ...
                 random_negative_prob=0.5,
                 interpolation='nearest'):
        assert isinstance(angle, tuple), 'The angle type must be tuple, but ' \
                                         f'got {type(angle)} instead.'
        ...
        self.random_negative_prob = random_negative_prob
        self.interpolation = interpolation
        self.return_translate = return_translate
    ...
    def __call__(self, results):
        ...
        # just return the translate
        # if self.return_translate:
        #     results['affine_matrix'] = torch.inverse(matrix_tran)[:2]
        matrix = matrix[None, :2, :]
        if self.keys is None:
            for key in results.get('img_fields', ['img']):
                img = results[key].float()
                c, h, w = img.shape

...
@PIPELINES.register_module()
class Binary_conv(object):
    def __init__(self,weight,keys):
        self.weight = weight
        self.keys = keys
        
   # def binconv(self):
     #   conv = torch.nn.Conv2d(3, 3, (11, 11), stride=1, padding=5, bias=False)
     #   conv.weight.data = self.weight
     #   conv = torch.nn.Conv2d(3, 3, (11, 11), stride=1, padding=5, bias=False)
     #   conv.weight.data = self.weight
     #   img = results['img']
     #   img_sim = conv(img)
     #   img_sim = Resize([112,96])
     #   results['img']=img_sim
    ...
    def __call__(self, results):
       ...
       # print(img.shape)
            img_sim = conv(img)
            img_sim=img_sim.detach()
       # img_sim.requires_grad_(False)
       # print(img_sim.shape)
            img_sim = F.interpolate(img_sim, scale_factor=2.24, mode='nearest')
       # resize = torchResize([112,96]) 
      #  img_sim = resize(img_sim)
            img_sim = img_sim.squeeze(0)
            img_sim = (img_sim-torch.min(img_sim))/(torch.max(img_sim)-torch.min(img_sim))
       # img_sim = img_sim.detach().numpy()
       # print(img_sim.shape)
            results[key]=img_sim
       # print('conv_over')
        return results
...
class Normalize_01(object):
    ...
    def __call__(self, results):
        for key in results.get('img_fields', ['img']):
            ...
            # normalize after 根据最大值归一化到0-1之间
            mmin = results[key].min()
            mmax = results[key].max()
            results[key] = (results[key]-mmin)/(mmax-mmin)
...
class StackImagePair(object):
    ...
    def __call__(self, results):
        if len(self.keys) == 1:
            results[self.out_key] = results[self.keys[0]]
        else:

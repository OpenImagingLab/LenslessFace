...
from mmrazor.datasets import build_hybrid_dataset
...
def train_hybrid_model(model,
    ...
    train_cfg = cfg.data.get('train_dataloader', {})
    train_data_loader = {}
    for key, ds in dataset[0].items():
        train_loader_cfg = {**loader_cfg, **(train_cfg.get(key, {}))}
        train_data_loader[key] = build_dataloader(ds, **train_loader_cfg)

    data_loaders = [train_data_loader]
    if len(dataset) > 1:
        data_loaders = data_loaders + [build_dataloader(ds, loader_cfg) for ds in dataset[1:]]
    ...
    # register eval hooks
    if validate:
        val_dataset = build_hybrid_dataset(cfg.data.val)
        # The specific dataloader settings
        val_loader_cfg = {
            **loader_cfg,
        ...
        eval_cfg['by_epoch'] = not(cfg.runner['type'] in ['IterBasedRunner', 'HybridIterBasedRunner'])
